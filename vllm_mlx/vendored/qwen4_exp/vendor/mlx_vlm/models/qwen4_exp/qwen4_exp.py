    ...
    def sanitize(self, weights):
        if get_ple_runtime_mode() == "mmap" and not getattr(
            ...
            for key in [key for key in weights if _NGRAM_STORAGE_RE.search(key)]:
                weights.pop(key)
        # Fork edit (vendored from oMLX): the FP8 checkpoint variant needs
        # oMLX's dequantize_fp8_weights, which this fork does not vendor.
        # BF16/affine-quantized MLX checkpoints (e.g. the REAP MLX builds)
        # carry no F8_E4M3 tensors and skip this path entirely.
        if any(key.endswith("weight_scale_inv") for key in weights):
            raise NotImplementedError(
                "qwen4_exp FP8 checkpoints are not supported by the vendored "
                "implementation; use a BF16 or MLX-quantized conversion."
            )
