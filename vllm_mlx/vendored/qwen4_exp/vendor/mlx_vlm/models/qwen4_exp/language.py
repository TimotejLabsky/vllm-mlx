...
try:
    from ..qwen3_5.language import _target_verify_linear, _target_verify_linears
except ImportError:
    # Fork edit: some mlx-vlm builds labelled 0.6.17 lack these. They are
    # pure performance shims (batched target-verify / fused decode linears);
    # the plain projection is semantically identical on every path, and the
    # fork never enables target-verify (MTP stays unbound).
    def _target_verify_linear(linear, x, target_verify):
        return linear(x)

    def _target_verify_linears(linears, x, target_verify):
...
def resolve_ple_runtime_mode(
    ...
    if requested not in {"auto", "resident", "mmap"}:
        raise ValueError("VLLM_MLX_QWEN4_PLE_MODE must be auto, resident, or mmap")
    if requested != "auto":
        return requested
...
def configure_ple_runtime(model_path: str | Path, mode: str | None = None) -> str:
    ...
    # Fork edit: fork-conventional env var first, oMLX's accepted as a
    # fallback so operator muscle memory from the oMLX taste-test still works.
    requested = (
        mode
        or os.environ.get("VLLM_MLX_QWEN4_PLE_MODE")
        or os.environ.get("OMLX_QWEN4_PLE_MODE")
    )
...
class Qwen4ExpDecoderLayer(nn.Module):
    ...
    def __call__(
        ...
        # Fork edit: gdn_sink/target_verify are MTP-capture kwargs that some
        # mlx-vlm qwen3_5 builds do not accept. They are only ever non-default
        # on MTP paths the fork never enables, so pass them only when set —
        # identical behaviour on new builds, no TypeError on old ones.
        _mtp_kwargs = {}
        if gdn_sink is not None:
            _mtp_kwargs["gdn_sink"] = gdn_sink
        if target_verify:
            _mtp_kwargs["target_verify"] = target_verify
        if self.is_linear:
        ...
        # Fork edit: same cross-build guard as above — Qwen3_5MoeSparseMoeBlock
        # in some mlx-vlm builds has no target_verify parameter.
        branch = self.mlp(mixed, **({"target_verify": True} if target_verify else {}))
        injection = branch[..., None, :] * injection_weights[..., None]
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if inputs_embeds is None and gdn_sink is None:
            ...
            if host is not None:
                # Fork edit: oMLX-internal MTP priming — unavailable and
                # unwanted here; skip rather than ImportError. (Reached even
                # without bind_mtp_owner: mlx.nn.Module attribute lookup can
                # produce a truthy host_ref on some builds.)
                try:
                    from omlx.patches.mlx_lm_mtp import prompt_priming
                except ImportError:
                    prompt_priming = None

                if prompt_priming is not None and prompt_priming.capture_eligible(host, cache):
