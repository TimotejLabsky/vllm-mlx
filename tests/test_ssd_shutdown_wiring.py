...
Adapted from upstream PR #634 (fork patch #49). `close_ssd_tier()` /
`SSDCacheTier.close()` used to be reachable only from `Scheduler.reset()` —
neither `EngineCore.stop()` (text engine) nor `MLLMScheduler.stop()` (MLLM
engine) ever closed the SSD tier, so every normal shutdown (= every
llama-swap model swap on the Studio) leaked the writer thread and silently
dropped any spills still sitting in the queue.

Fork extension: `Scheduler.close_ssd_tier()` also drains the BATCHED
system-KV cache's own SSD writer (patch #36's tier) on the same lifecycle —
since the 2026-07 fleet flip every route runs that tier.
"""
...
class TestEngineCoreStopClosesSSDTier:
    ...
    async def test_stop_closes_batched_system_kv_writer(self):
        """Fork (#49): the batched cache's own SSD writer (patch #36) must
        drain on the same lifecycle — every fleet route runs it now."""
        from vllm_mlx.engine_core import EngineCore
