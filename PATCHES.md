...
> **2026-09-23 rebase note — rebased onto upstream `93b0b71` (10 commits past `ec8e493`, includes `v0.5.0` `b064502`); suite green at 3951 passed / 31 skipped / 30 deselected, ruff clean.** 190 fork commits replayed, 0 dropped, 4 conflict stops (`mllm_scheduler.py`, `mllm_batch_generator.py`, `tests/test_normalize_messages.py`, `ci.yml`) — exactly the files `git merge-tree` named. Backup: `backup/pre-rebase-2026-09-23`. Net change vs `backup/…` equals upstream's window diff in 31 of 39 files; the other 8 differ exactly by the resolutions below.
> - **Upstream #774 collides with patch #83 — and the dangerous half AUTO-MERGED.** `_normalize_messages` in `server.py` merged without a conflict marker into upstream's condition *plus* #83's structural carry; upstream's `not prev.get("tool_calls") and not msg.get("tool_calls")` would have silently disabled #83 (only the test file stopped). Resolved in #83's replay: #774's role allowlist kept (it is the #109 fix), the tool_calls guard dropped, and upstream's `test_assistant_tool_calls_are_not_merged_into_text` dropped with it (asserts the opposite of #83). Pinned by `test_83_109_…` in `tests/test_fork_invariants.py`.
> - **#772 `reasoning_effort` forwarding vs patch #76 — fork semantics kept.** Upstream forwards the raw value (incl. `"none"`) via `_resolve_chat_template_kwargs(request_reasoning_effort=…)`; the call-site argument is dropped (the helper keeps the parameter), so #76's block stays the only writer. Upstream's precedence test (server default < effort < explicit kwargs) passes unchanged in order; it now also expects #76's fallback key. #771's `Message.reasoning_content` and #772's `ChatCompletionRequest.reasoning_effort` duplicated fork fields (later definitions shadowed them) and were removed.
> - **Patch #54 code RETIRED into upstream #779 (`d23d65b`)** — the same `batch_generator.vision_cache` fix; upstream's version taken, the fork's 5 regression tests kept and green against it.
> - **#694 (`e81dbb4`) fail invalid media atomically:** its `try/except`-removal met #58's `default_video_fps`/`max_video_frames` wiring (both kept). Its inline-preprocess error path is live, but its test drove it with audio requests, which the fork never inlines (`be3f08f` skips `has_media`; media gets its own prefill, where `_process_prompts` already fails each request atomically) — the test is driven by failing text requests instead, and the inline error response carries #104's `error_kind`.
> - **`ci.yml`:** the stop was resolved to the fork file wholesale, which dropped upstream's new unit-test entries, the streaming text-with-tool-calls job and `scripts/client_acceptance/` in ruff; the `restore:` commit rebuilds it as upstream's file + the fork's changed-lines black step only.
> - **#690 (`2461fc4`) text alongside tool calls** auto-merged at both fork OpenAI streaming sites and the Responses/Anthropic paths, verbatim; the `leading` chunk bypasses nothing the fork adds (the only later hook, `_record_structured_stream_content`, is response_format-only). **#646/`8eae22e`** Anthropic mid-conversation system messages now fold into the leading system — a template crash before; it changes the system prefix on those turns (system-KV miss), which beats a 500.
> - **Net-new upstream, inert or additive here:** `b064502` v0.5.0 (SimpleEngine completes the prefix store before aborting the producer — the fork's second `abort_event.set()` site is SpecPrefill, left as upstream left it, since there `finished` includes stop-string hits); `93b0b71` qwen_tool_parser final-delta emits only newly completed calls (hermes routes unaffected); `700189b` (#789) dead-helper removal (no fork users); client-acceptance harness + CI workflow.

> **2026-09-15 rebase note — rebased onto upstream `ec8e493` (11 commits past `22efb47`); suite green at 3601 passed / 31 skipped / 30 deselected, ruff clean.** 172 fork commits replayed, 0 dropped, 4 conflict stops. Backup: `backup/pre-rebase-2026-09-15` (on origin).
> - **Conflict surface measured before rebasing** (`git merge-tree --write-tree --name-only main upstream/main`) and it named the four stop files exactly: `text_model_from_vlm.py`, `cli.py`, `tests/test_server.py`, `tests/test_batched_engine_mllm_config.py`. Both test files were append-vs-append at EOF (both sides kept).
> - **Verified hunk-by-hunk, not by seam counts:** the rebase's net change (`backup/pre-rebase-2026-09-15` → tip, before the `restore:` commit) matches upstream's window diff (`22efb47` → `ec8e493`) hunk-for-hunk in 34 of 38 touched files, and the other four differ exactly by the resolutions below. So every fork line outside upstream's hunks is byte-unchanged, and every upstream line arrived verbatim — including the large auto-merges into `server.py`, `scheduler.py`, `ssd_cache.py` and `output_collector.py`.
> - **Patch #95 (`prefill-step-size-threading`) PARTIALLY RETIRED — upstream #729 (`d80db21`) merged the same fix.** Upstream threads `prefill_step_size` into `serve_command`'s `SchedulerConfig` (plus the registry clone and positive-int validation); the serve-site conflict was only our comment, resolved to upstream. The `bench_command` site remains fork-only, and our serve-path regression test stays alongside upstream's.
> - **Upstream #741 (`761abfe`, `qwen4_exp` stays on the mlx-vlm text path) — lost in the replay, re-applied in the `restore:` commit.** `e9108e8` rewrote the dispatch region, so every stop in `text_model_from_vlm.py` was resolved to the fork commit's own version (exact, because #741 is the only upstream delta in that file in this window), and the guard was re-added on top of the fork's candidate chain. Redundant today — `_import_text_model_classes` already raises for `qwen4_exp` since mlx-lm has no such module — but **load-bearing for #97**: the day mlx-lm ships a `qwen4_exp` module, the fork would build an mlx-lm TextModel with a resident ~29 GB PLE table instead of the vendored SSD-backed one. Upstream's two dispatch tests caught the omission.
> - **Upstream #740 (`d843a76`) carries four fixes; two needed fork adjustments:**
>   - *Implicit `<think>` streaming.* `_prepare_streaming_reasoning_parser` now probes the chat template and calls `reset_state(implicit_mode=...)`. **The fork's #27/#47 latch sites (Responses and Anthropic streaming) build their parser directly and would silently have missed it** — now wired, gated on thinking ON exactly like upstream (which never builds a parser with thinking off), so the #27 latch still treats untagged text as content. **Fleet impact: `GLM-4.7-Flash-4bit` changes behaviour** — its template ends the generation prompt in `<think>` (checked against the HF template), so streamed chain-of-thought now lands in `reasoning_content` / thinking blocks instead of being glued to the answer, on all three streaming APIs. `GLM-4.6V-Flash` is unaffected (its template opens no `<think>`), and only the glm4 parser reads the flag. One test stub was updated to the new `reset_state` signature (the stub was fixed, not the call). Three new tests; the two thinking-on ones fail with the probe disabled (mutation-checked).
>   - *`SSDIndex._SCHEMA_VERSION` 1 → 2 (CacheList serializer).* A version mismatch drops every row. **`system_kv_ssd.py` reuses `SSDIndex`**, so the first start after deploy would have purged every route's system-KV spills (and reconcile would then delete the data dirs as orphans) even though our MLX-native entry format did not change. Pinned via `_SystemKVIndex` (`_SCHEMA_VERSION = 1`); the test seeds a version-1 index as the deployed build leaves it and fails with the pin removed (mutation-checked). Upstream's own `SSDCacheTier` still migrates — no fleet route sets `--ssd-cache-dir`, so nothing else is purged.
>   - *`--stream-interval > 1` token loss* (collector `notify=False` gating) — inert here: no route sets `--stream-interval`. It merged next to the fork's `_merge_outputs` fields (#82 `cached_tokens`, #89 `grammar_unterminated`, `error_kind`), all intact.
>   - *bf16 `ValueError` upcast, unmaterialized-KVCache guard, CacheList spill* — BatchedEngine `ssd_cache.py` tier only; taken as-is.
> - **Net-new upstream, inert or additive here:** `d7632b0` (#713) MLLM prefix-cache percent + registry memory budgets; `37c43ab` (#739) registry-mode `/metrics` (no registry routes); `f2d3ad3` (#737) MTP install guard (MTP off fleet-wide); `ec8e493` (#767) the Anthropic adapter now uses the shared line-anchored billing-header stripper (a real fix — the old unanchored regex could delete user text mid-sentence); `acb37a5` (#751) dead-helper removal (unused in the fork too); `fe108f1` / `23fdc45` test hygiene (deselection count unchanged at 30); `a9204ad` CI auto-approve workflow.
...
> - **`rerere` REPLAYED A BAD RESOLUTION — new lesson.** The 08-22 attempt recorded a resolution for `mllm_batch_generator.py` that still contained a stray `>>>>>>>` marker; rerere faithfully re-applied it, and the file was *not* listed as unmerged, so nothing but a whole-tree marker scan would have caught it. Purged with `git rerere forget <path>`. **rerere caches mistakes as happily as fixes** — always scan the tree for markers before `--continue`, not just the unmerged list.
> - **Patch #44 (`engine-core-idle-backoff`) is RETIRED but its commit still replays as docs-only.** Upstream's merged `5ed6554` (#552) is a superset — one `step_interval` knob plus an event-driven idle wait that wakes on `add_request` — so every code hunk resolved to upstream and the commit now touches only `PATCHES.md`. Verified: no `idle_step_interval` anywhere in the tree.
> - **Wanted upstream delta restored again: `mx.clear_cache()` in `SimpleEngine.stop()`.** Dropping Python references frees objects but not MLX's retained Metal buffers, so an idle-unload reclaims no device memory — load-bearing since upstream #638. Upstream's `test_stop_calls_mx_clear_cache` is the guard, and it is what caught the omission both times.
> - **Upstream #669 (`4b654c0`, MLLM assistant drafters) — accepted on BatchedEngine, REFUSED on SimpleEngine.** `server.py` forwards `default_mllm_draft=` to both engines, so the rejected `simple.py` would `TypeError` on every start. Same reject-or-implement rule as `prefix_trie_cache` (#574): the constructor accepts the kwarg so shared plumbing works, and **raises if it is enabled** rather than silently ignoring it. Upstream's three SimpleEngine drafter tests are skip-marked fork-hygiene; BatchedEngine implements the feature and is what every fleet route uses. MTP/drafters are off fleet-wide regardless.
> - **Duck-typing guards for upstream tests that build partial objects** (behaviour unchanged): `_default_mllm_draft` and `_mllm_draft_model` reads in `batched.py`, `hybrid_kv` in `Scheduler.close_ssd_tier`, and `_pressure`/`prefill_batch_size` in `_select_prefill_batch`. Fork tests construct engines via `__new__` and set only what they exercise.
> - **Test-side only:** three upstream prefix-cache stubs gained `has_media=False` — **the stubs were fixed, not the guard**, since defaulting `has_media` in production would make a safety guard fail *open*. Seven fork-hygiene skips total, each with a reason.
> - **Silent merges verified, not trusted:** seam counts unchanged from the previously-verified rebase — `scheduler.py` 22 `hybrid_kv` / 23 `ssd_tier`, `batched.py` 4 `raw_text` / 4 `force_text_only`, `mllm_batch_generator.py` 19 `rope_delta` / 9 `has_media` / 12 pressure, `EFFORT_FALLBACK_KEY` (#76) in all three files, #77 repetition-stop present, **#79 prompt-skip present (8 refs)**, #49's `hybrid.close()` drain intact.
> - **Net-new upstream gained:** `4b654c0` (#669) MLLM assistant drafters on the batched path, plus everything from the 08-22 window: `b281bee` (#714) system-prefix probe from normalized messages, `bf5ccb4`/`8c814e3` MLLM prefix-cache ownership and rewind safety, `515dd14` (#636) fail-closed structured output, `ad8da1d` (#638) idle-unload memory release, `7afa61a` (#640) CLI memory-budget override, `61fab4b` (#598) Step3p5 parser, `c771d2a` (#543) VLM TextModel owner thread, `b135cd8` (#643) shield cancellation noise, `7c7232a` (#693) paged-cache bulk release.
...
## 44. `patch: engine-core-idle-backoff` — cherry-pick of upstream #552 + abort symmetry — **RETIRED (in base as of `4b654c0`)**
...
**Rebase 2026-09-25 (upstream #749):** upstream fixed the same dark `steps_executed` on the MLLM path (their #746) with its own `_steps_executed` counter, incremented only when `step()` returns without raising. It auto-merged with NO conflict next to ours, which left the key twice in `get_stats()` (the later one, ours, silently won) and twice in the engine promote tuple. Ours is retired in favour of upstream's; this patch no longer touches `steps_executed`.
...
- **From the 2026-09-16..21 memory-safety series (#103–#108)** — status and evidence in [`docs/fork/improvement-plan-2026-09-16.md`](docs/fork/improvement-plan-2026-09-16.md); none urgent:
  - **Verify "warm == cold byte-identical" on the 27B for multi-turn chains.** On hybrid `Qwen3.5-4B-4bit` a later turn of an all-restores pass differed from the first pass ~20 tokens in, *identically on unpatched code*: a different restore position means different prefill chunk boundaries through the GDN recurrence → ~1e-6 drift → an argmax tie flips. Run the standard gate recipe once on the 27B before leaning on the claim.
  - **Exempt the first post-system message boundary from `boundary_min_step` thinning (#88).** Requests sharing only a ~1K-token system prompt miss entirely: the boundary is thinned and the uniform interval is 2048, so no checkpoint ≤ the LCP exists. (A 3.3K system prompt is unaffected — it has the 2048 checkpoint.)
  - **SSD checkpoint dedup.** Every entry under one system prompt re-serialises the same ladder (~0.3 GB each on the 27B; a 4.4K-token entry is 306 MB bf16 KV + 906 MB fp32 GDN state = 151 MB × 6 copies). RAM shares these by reference; disk does not. Do **not** reach for bf16 state — it would fail the T=0 gate.
  - **Spill-queue cap vs three deep chains finishing together.** 9 GB fits two 60K entries; the third is dropped (seen once at the end of stress run 3). Raise to ~13 only if `MacStudioLLMSpillsDroppedNotWritten` shows it in real traffic.
  - **#104 residuals:** streaming `PromptTooLong` still arrives as 200 + an error frame (see the next item); an out-of-band `/cancel` ends without a terminal chunk.
  - **#105/#106 calibration for other routes:** the solo guard and the projected gate are armed only on Qwen3.8-27B-4bit. The 8-bit sibling needs the same env block; the 45 GB-weight class (Coder-Next / Next-80B) needs its chunk transient and bytes/token *measured* first — that is where the guard would actually bound (61.2 − 45 − transient leaves single-digit GB for KV).
  - **#107's pin and lazy-SSD-fallback paths have never fired in live traffic** (three stress runs: every deep hit came via the ordinary SSD promote, because the RAM bag had already turned over by enqueue). They are covered by tests and the real-server e2e; treat a first live `lazy_ssd_fallbacks` / `pinned_entries` > 0 as worth a look, not as proven.
  - **Cross-process memory guard:** not built — downgraded by evidence (the 09-16 storm was not cross-process; llama-swap ping-pongs the HA 35B and the 27B, they never co-reside). Revisit only on an observed co-resident OOM.
...
**Rebase force-pushes (2026-09-23).** A rebase makes `github.event.before` a commit that is not an ancestor of HEAD, and `before...HEAD` measures from the merge-base — the *old* upstream base — so every fork patch line counted as changed and `lint` went red on the rebase push (43 files, drift nobody added). When the base is not an ancestor, the script now diffs the two trees (`before..HEAD`): exactly what the push changed in content, i.e. upstream's window plus the conflict resolutions — drift a resolution introduces is still caught. Verified on the actual 2026-09-23 rebase (`backup/pre-rebase-2026-09-23` → `c122916`: red before, clean after) and pinned by `tests/test_black_changed_lines.py` (throwaway repo: rebase with pre-existing drift passes, rebase adding drift fails, ordinary push unchanged).

**On-box suite runs clean (2026-09-23).** Six tests failed on the Studio only, for environment reasons: five in `test_mcp_security` built configs for `npx`/`node`, which the Studio doesn't have (a module fixture now resolves whitelisted commands when absent — the tests check validation rules, not the host's PATH), and `test_106_a` compared cache state as flat arrays, while at mlx-lm `f4f3b57` (prod) `ArraysCache.state` is `(cache_list, left_padding, lengths)` (now compared recursively). **Note: a dev venv still on mlx-lm `9acef5f` tests the old state shape — keep it on the prod pin.**
...
**Follow-up 2026-08-20 — resolution order corrected to "operator floor beats neighbour".** The first cut mapped an unsupported level onto its nearest neighbour, so on Qwen3.8 `high -> xhigh`. Two defects surfaced once the route carried `--default-chat-template-kwargs '{"reasoning_effort":"medium"}'`:

1. `xhigh` **is** Qwen3.8's template default, so `high` was observationally identical to sending nothing — and Claude Code sends `high` on every request. For the fork's most important client the patch was a **no-op**, leaving in place exactly the runaway-thinking mode it was written to prevent.
2. Worse, the engine's *drop* landed on the **template** default rather than the route's floor, so a garbage value bought **more** thinking than sending nothing (measured: `banana` -> 68 prompt tokens = xhigh, vs no-effort -> 26 = medium). The floor was silently defeated by any unsupported value.

Field check settled the fix: **no engine maps values.** llama.cpp forwards verbatim (and 500s); vLLM converts the top-level param to an `enable_thinking` **bool** and tells you to set the level through `chat_template_kwargs` / `--default-chat-template-kwargs` ([vLLM's own Qwen3.8 recipe](https://recipes.vllm.ai/Qwen/Qwen3.8-27B)); SGLang doesn't validate. `high -> xhigh` had no precedent — it was ours. So the resolution order is now vLLM-aligned:

    exact match -> route floor -> nearest neighbour -> drop

The floor is plumbed engine-ward through a reserved `chat_template_kwargs` key (`EFFORT_FALLBACK_KEY`), because only the server can read `--default-chat-template-kwargs` and only the engine can read the template. `normalize_effort_in_template_kwargs` pops it unconditionally so it can never reach the Jinja context; `strip_effort_fallback` guards the mlx-vlm MLLM passthrough, which never calls the normalizer. `render_with_effort_fallback` walks the same ladder (floor, then bare) so unparseable templates resolve identically. Neighbour mapping survives for routes with no floor configured (`xhigh -> high` on gpt-oss). Verified live: `high` and `banana` both now render medium (26 prompt tokens); `low`/`xhigh` unchanged; `none` still `enable_thinking=False`. 15 further tests, suite **2870 passed / 24 skipped / 30 deselected**.
...
## 77. `patch: repetition-detection-stop` — end degenerate loops at the scheduler stop-check

**Files:** `vllm_mlx/repetition_stop.py` (new), `vllm_mlx/scheduler.py` (consumer-level enforcement in `_process_batch_responses`), `vllm_mlx/metrics.py`, `tests/test_repetition_stop.py` (new)

Qwen3.8-27B degenerates in agentic sessions into exact repeating token cycles ("items items items…", duplicated tool-call retries, multi-line loops) and burns to `max_tokens` (opencode cap 16,384) on every degenerate turn. Sampler penalties categorically cannot end this: DRY/repetition_penalty reshape logits and forbid *forms*, so the model swaps near-variant tokens ("its", "itemsThe") and stays in the attractor — verified live 2026-08-22 with DRY 0.8/1.75/4/8192 + thinking budget 6144 + the froggeric fixed chat template all armed simultaneously. Note `\n` is a canonical DRY sequence breaker, so DRY can *never* even see a multi-line cycle; this detector has no breakers and catches them.

**What the patch does.** The scheduler's `_process_batch_responses` — the per-token response consumer, alongside the EOS/`max_tokens` finishes it already maps — feeds each generated token into a per-uid ring buffer (`window` 512) and, every `interval` (16) tokens past `min_tokens` (64), scans the tail for the smallest period `p ∈ [min_period, max_period]` (1–128) whose cycle repeats `≥ min_repeats` (3) consecutive exact times covering `≥ min_span` (48) tokens. Block comparison runs from the end, making detection rotation-invariant (an ongoing loop triggers regardless of where in the cycle the last token landed). The `min_span` gate scales strictness inversely with period: a period-1 run needs 48 identical tokens, a 16-token cycle needs 3 copies — legitimate enumerations and code boilerplate are not exactly periodic for 48 consecutive tokens. On a hit the request finishes as `finish_reason="stop"` (OpenAI-client-safe), logs `[repetition-stop] uid=… period=… repeats=…`, and increments `vllm_mlx_repetition_stops_total`. On a hit the request is force-finished exactly like an abort (`batch_generator.remove([uid])`; uid-mapping cleanup follows the normal finished path). Buffers are discarded on finish, on abort, and on scheduler reset/stop.

**Layout gotcha (cost a deploy round).** The first cut hooked `_generation_step` inside `_install_chunked_prefill` — which only installs on the *legacy* mlx-lm BatchGenerator API. On the **native** chunked-prefill layout (what production runs) the fork just sets `prefill_step_size` and mlx-lm's own step runs, so the hook was dead code: a logit_bias-forced single-token run went straight to `finish=length` at 1024 tokens with the counter at 0. Enforcement therefore lives at the CONSUMER (`_process_batch_responses`), which both layouts share; `tests/test_repetition_stop.py::test_consumer_forces_stop_on_repeating_stream` drives the real consumer with a bare Scheduler and would have caught this. Generated tokens only — the prompt is never scanned. Cost: pure-Python list-slice compares (C speed), ~8K int compares worst case per check per sequence every 16 tokens; the perf-smoke test bounds one check at <10ms with realistic (<1ms) margins; disabled (default) it is a single attribute check per token.

**Prior art.** Direct analogue of vLLM main's `SamplingParams.repetition_detection` (`vllm/v1/core/sched/utils.py::check_sequence_repetition`, `stop_reason="repetition_detected"`); parameters follow the ThinkingLoopBreaker proposal (vllm-project/vllm#52673: window 512, period 4–128, ≥3 exact repeats, check every 16 tokens). Deliberately NOT think-section-scoped: the 2026-08-21 audit verified the thinking budget works and the loop continues in the *content* stream after `</think>`, so the whole-output stop is the shape that fits. Semantic loops that reword every cycle remain out of scope — nothing token-level catches those.
...
> **Status (2026-09-15 rebase): PARTIALLY RETIRED.** Upstream merged the same
> fix as `d80db21` (#729) — the `serve_command` site is now upstream's line
> (plus registry-clone threading and positive-int validation). Still fork-only:
> the `bench_command` site and this patch's serve-path regression test.
...
## 103. `patch: unaccounted-memory` — what the bag doesn't count must not pin the route at its watermark

**Files:** `vllm_mlx/scheduler.py` (recovery + two `list(self.waiting)`), `vllm_mlx/batched_system_kv.py` (promote snapshot, relief ordering, SSD wiring, stats, refusal reason), `vllm_mlx/system_kv_ssd.py` (`drop_backlog`, bounded busy-write, writer ref release, stats), `vllm_mlx/memory_pressure.py` (`before_evict` hook), `vllm_mlx/metrics.py` (+2 gauges), `tests/test_fork_invariants.py` (new, 11).

**Found live 2026-09-16/17** on Qwen3.8-27B-4bit under idealplace opencode CI. `/v1/status` at 09:18 on 09-17: **bag 0 entries / 0 MB, running 1, waiting 0 — active 52–54 GB** (one deep row on 15 GB of weights is ~25 GB). 51 Metal OOMs since midnight, `pressure_cache_clears` 3,864 in 67K steps, the bag wiped 61 times, a reviewer turn at 651 s. The 09-16 storm (14:47–15:52Z, 73 OOMs, peaks to 71 GB at `running=1`) had been attributed to a co-resident 35B; the 35B's own log shows it was not loaded in that window. Every guard (#48 relief, #101 budget, #102 gate) prices the *bag* — and three holders sit outside it:

1. **The SSD spill backlog (~12 GB, the big one).** The writer defers every write until `idle_check()`; on the batched path idle = `not scheduler.running` (`scheduler.py`), and a route under agent load is never idle. Log: backlog at **11.5 GB 23 minutes after spawn, 11.5–12.0 GB for the next 3 h; 4 spills written, 30 dropped.** The queue holds its own references to the flattened arrays, so relief evicting the mirrored bag entry frees nothing — relief burned the useful cache while the dead weight stayed. The 12 GB cap and idle-only policy were sized for SimpleEngine's turn gaps (2026-06-12).
2. **Recovery leaked checkpoint ladders.** `_recover_from_generation_error` aborts rows but never called `hybrid_kv.discard_pending` (the normal abort path does), and the `except` branch `break`s past `_cleanup_finished`. Each aborted row kept up to 8 × 146.8 MiB of recurrent state; a row that HIT holds references to its **donor's** checkpoint arrays (`fetch` installs `list(inherited)`), so the leak also pinned evicted entries. Signature: post-recovery baseline ratcheting `55.9 → 57.2 → 63.0 → 65.6 → 67.1 → 68.4 → 69.0 → 70.8 → 71.2 GB` across the storm, relief logging "evicted 1 snapshot entry" nine times.
3. **The writer's loop locals** kept the last-written snapshot referenced until the *next* spill arrived (indefinitely on an empty queue) — one entry, 1–4 GB.

Same incident, separate bug: `promote_ssd_pending` iterated the live `scheduler.waiting` deque on the executor while `add_request` appends on the event loop → `RuntimeError: deque mutated during iteration` ×15 on 09-16, each one killing the batch generator through the leaking recovery.

**Fix.**
- **Recovery** mirrors `_do_abort_request`: drops `prompt_cache` / `_extracted_cache` and discards the pending ladder for every aborted id.
- **Deque:** `list(scheduler.waiting)` in the fork seam, plus the two same-shaped iterations in `scheduler.py` (upstream's `_try_promote_ssd_pending`, and the status handler, which reads the deque from the event loop).
- **Backlog is relievable.** `SystemKVSSDStore.drop_backlog()` drains the queue *and* the spill the writer is holding in its idle-wait (generation bump + wake event). `PressureManager.relieve` gains an optional `before_evict` hook; the bag passes the backlog drop, so a breach the backlog can absorb costs **no** cache entry. Not counted as an eviction (`pressure_backlog_drops`, SSD `backlog_drops`).
- **Backlog is bounded for batched routes.** `VLLM_MLX_SSD_SYSTEM_KV_MAX_QUEUED_GB` (batched default **4**, was a hardcoded 12; an empty queue still admits one oversized spill) and `VLLM_MLX_SSD_SYSTEM_KV_BUSY_WRITE_S` (batched default **30**; 0 = never): after that wait a queued spill is written while busy, but only when `not under_pressure()`. SimpleEngine keeps both old behaviours (config defaults unchanged, pinned by a test).
- **Writer releases** `item` / `tensors` after every iteration.
- **Visible from now on:** `/v1/status` cache block gains `pending_ladders`, `pending_ladder_mb`, `pressure_backlog_drops`; the `ssd` block gains `queued_bytes`, `queue_depth`, `backlog_drops`, `busy_writes`. Prometheus: `vllm_mlx_cache_pending_ladder_bytes`, `vllm_mlx_cache_ssd_queued_bytes`. The store-refusal line now carries `reason=over_watermark|copy_would_overshoot`.

**What this does NOT change:** no threshold moves. Watermark 85, `KV_BUDGET_MB=4096`, the #101 reserve and `MAX_PROMPT_TOKENS` were all tuned on a process carrying 12–25 GB of unaccounted memory — re-ladder them on a clean process before touching any (plan: `docs/fork/improvement-plan-2026-09-16.md`, P1-2).

**Upstream:** N/A — fork-owned modules; the two `list(self.waiting)` one-liners are upstreamable on their own.

**Verification:**
- **Red then green.** `tests/test_fork_invariants.py`: all 11 fail with `vllm_mlx/` stashed to `5edda49`, all pass patched. Recovery through the helper *and* through the real `step()` except-branch (mock generator raising the Metal OOM string → `finish_reason="error"`, `pending_ladders == 0`); promote hook surviving a racing `waiting.append`; `drop_backlog` releasing queued + held spills with the writer still alive afterwards; busy-write gated on the memory check; writer frame locals `None` after a write; relief dropping the backlog with **0 evictions**, and still evicting when the backlog is empty; batched env defaults/overrides; both gauges through `render_metrics`.
- **Full suite:** 3693 passed / 31 skipped / 30 deselected (#102 baseline 3682 + 11). `ruff` clean.
- **Live gate after deploy** (idle window — a config reload restarts every model): under the same CI load, `ssd.queued_bytes` ≤ 4 GB and draining (`busy_writes` > 0), `pending_ladders == 0` whenever `num_running + num_waiting == 0`, the `[Metal memory] active=` line after any `[generation_error_recovery]` no higher than before the request, `entry_count` > 0 sustained, `pressure_cache_clears` per 1K steps down by an order of magnitude from 57.

## 104. `patch: recovery-error-signalling` — an engine-side abort must not read as an answer

**Files:** `vllm_mlx/engine/base.py` (`GenerationAborted`, `raise_if_generation_aborted`), `vllm_mlx/engine/batched.py` (4 output sites), `vllm_mlx/scheduler.py` (`error_kind` + recovery counters), `vllm_mlx/server.py` (503 translation, 503-shaped stream frames, status keys), `tests/test_fork_invariants.py` (+8).

**Found 2026-09-16** in the Qwen3.8-27B-4bit route log: **2,266 responses, 100% HTTP 200** — including 105 Metal-OOM recoveries and 16 zero-token streams. `generation_error_recovery` reports each aborted row as an ordinary finished output with `finish_reason="error"` and no exception crosses any layer, so:
- **streaming:** an empty delta stamped `finish_reason:"error"`, a zero usage chunk, `[DONE]` — #91's `_stream_error_chunk` lives on the `except` branch this path never reaches; `result_label` stayed `"success"`, so `stream_aborts_total` was **not** incremented, and a mid-stream OOM after N tokens was metered as a successful request;
- **non-streaming:** `ChatCompletionResponse(content=None, finish_reason="error")`, status 200.

No OpenAI client checks `finish_reason == "error"`. LiteLLM (`num_retries: 0` in our config) passes it through as a clean turn; opencode retries only on status ≥ 500 or a message matching `429|5xx|overloaded|service unavailable|internal error`, and never on empty output — a dead reviewer turn and an empty answer were indistinguishable. (#103 removed the *cause* of that day's OOMs; this makes the next abort, whatever causes it, visible.)

**Fix — translate once, at the engine layer.**
- `RequestOutput.error_kind` already existed (MLLM `prompt_too_long`). Recovery now stamps `"oom"` (Metal `Insufficient Memory`) or `"generation_error"`, and counts `generation_recoveries` / `recovery_aborted_requests` (scheduler stats → `/v1/status`).
- `raise_if_generation_aborted(output)` at the four `BatchedEngine` sites where scheduler outputs become `GenerationOutput` (text + MLLM, stream + non-stream) raises the typed `GenerationAborted`. `prompt_too_long` is excluded — it keeps its non-retryable 400 path. Good chunks already emitted still reach the client; the abort never does as text.
- **Server, non-streaming:** `except GenerationAborted` beside every `EngineBusy` handler → **HTTP 503 + `Retry-After: 15`**, `detail={error: generation_aborted, kind, message}`. 503 rather than 429: the request was fine, the engine was not, and 429 carries rate-limit cooldown semantics in LiteLLM.
- **Server, streaming** (headers long gone): a dedicated branch ahead of #91's generic one emits the vLLM/LiteLLM error frame in a **503 shape** — `{"error": {"message": "Service Unavailable (503): …", "type": "service_unavailable", "code": 503, "error": "generation_aborted", "kind": "oom", "retry_after": 15}}` then `[DONE]`; Anthropic dialect `overloaded_error`. Logged as one WARNING (the scheduler already logged the traceback), does **not** re-raise (an expected condition should not print an ASGI traceback), `result_label="error"` so `stream_aborts_total{phase}` and `inference_requests_total{result="error"}` finally count it. #91's generic frame is untouched.

**Deliberately not in this patch:** no engine-side retry (the recovery comment is right — the request that OOMed would OOM again; the *client* backs off); streaming `PromptTooLong` still arrives as a 200 + error frame (pre-tokenizing in the pre-stream probe costs a template render per request; `prompt_rejections` is 0 fleet-wide because the gateway caps first); out-of-band `/cancel` still ends without a terminal chunk. Both recorded in `docs/fork/improvement-plan-2026-09-16.md`.

**Upstream:** the `finish_reason="error"` → 200 shape is upstream's too; `GenerationAborted` + the 503 frame are a clean candidate for the upstreaming branches.
...
## 106. `patch: projected-cobatch-admission` — queued restores and co-batch admission see real memory

**Files:** `vllm_mlx/batched_system_kv.py` (`peek`, `restore_bytes`, `projected_cobatch_verdict`, gate 4 in `should_defer_cobatch`, lazy `fetch_for_request`, `materialize_pending_restore`, envs, stats), `vllm_mlx/scheduler.py` (one hook in `_schedule_waiting`), `tests/test_fork_invariants.py` (+9), `scripts/fork/e2e_lazy_restore.py` (new).

**Found live 2026-09-17**, 2.5 h after `VLLM_MLX_BATCHED_KV_BUDGET_MB` went 4096 → 8192 on Qwen3.8-27B-4bit: **three Metal OOMs (12:44, 12:47, 13:59 local), 6 requests aborted, peaks 63.0–63.6 GB.** The load was abnormal — 13 idealplace review runs + 1 dispatch stacked concurrently, all cancelled at 14:01 when reviews became opt-in — but the mechanism is general, and all three had one signature:

1. a ~65–70K-token row decoding at ~40–42 GB active (the #101 dynamic bag grown to ~20 GB of idle headroom);
2. a second deep row co-batched (8192 MB allows 2 × 65K);
3. a **67–72K-token cache restore realised for a request that was still WAITING** (`restore at 67566/69895 … prefilling 2329`) — OOM 17–28 s later.

The admission gates (#39/#40/#102) price *padded KV against a budget* and gate 3 looks at the present; **none asks what this admission adds to what the process already holds.** They are blind to (a) the bag's current size, (b) the merge-time padded copy, and (c) restored caches held by queued requests: `add_request` built the full restored copy (4–5 GB at 70K) the moment a request arrived, so every queued request pinned its restore for its whole wait, in no budget — three waiting ≈ 13 GB. And #48 relief acts only *between* steps, so "the bag can be shed" does not protect a single step. (The "≈30 GB worst case" reasoning behind the 8192 change assumed exactly that, and was wrong.) What held: #103 (no ratchet — active fell back to 29 → 18 GB), #104 (6 `aborted by the engine` warnings: clients got the 503 frame), and `MacStudioLLMGenerationRecovery` fired.

**Fix — two env-gated parts, both inert by default.**

- **`VLLM_MLX_BATCHED_LAZY_RESTORE=1`.** `add_request` only **matches** (`peek`: same LCP + `select_restore_pos` as `fetch`, no state slices, no copy, no counters) and marks the request `system_kv_pending` with `cached_tokens` / `remaining_tokens` set. `peek` LRU-touches the entry, so the chain a queued request is about to extend outlives an idle one while it waits. The copy is built by `materialize_pending_restore`, called from `_schedule_waiting` **past every gate**, on the executor thread — the thread that steps the batch, so the slices are recorded and evaluated where they are used (the #28 stream/thread class cannot arise; the eager path's cross-thread realise is what needed the workaround). An entry evicted during the wait degrades to a shallower hit or a miss (`lazy_restore_misses`); nothing else changes. Follows #36's `ssd_pending` pattern.
- **`VLLM_MLX_BATCHED_PROJECTED_ADMISSION=1`** (shares #105's `SOLO_TRANSIENT_MB` / `SOLO_CEILING_PCT`). Gate 4 in `should_defer_cobatch`: `active + merged padded KV at B+1 seats + the new row's prefill growth + its not-yet-built restore + one chunk's transient` vs the OOM wall. Over it, pure cache goes first — SSD spill backlog, then bag entries LRU-first **but only down to the RAM floor** (a second seat is not worth an empty cache) — and the projection is re-checked. **Only if shedding can actually close the gap**: when `active + need − sheddable` is still over the limit nothing is shed, because the bag holds the very entry this request will restore from (a test caught the first cut evicting the whole bag and *then* deferring). What does not fit **waits, never rejects** — rows finish and free their KV, so deferral always makes progress. Fails open.

Stats: `lazy_restores`, `lazy_restore_misses`, `projected_defers`, `projected_relief_passes`.

**Upstream:** fork-owned (rides the #34-series branch).

**Verification:**
- **Red then green.** 9 tests in `tests/test_fork_invariants.py`, all red with `vllm_mlx/` at `eb91d2e`: `peek` equals `fetch`'s position yet builds/counts nothing and LRU-touches; a lazy `add_request` pins no memory until admission; **a lazy restore is byte-identical to an eager one** (layer-by-layer `mx.array_equal`), and eager stays the default; eviction during the wait → miss; projection inert unless armed; **the 09-17 signature** (mocked allocator) makes room with one entry shed and admits; what cannot fit waits with the cache intact and is never rejected; shedding stops at the RAM floor; and through the real `_schedule_waiting` a deferred request is *not* materialised while an admitted one is.
- **Full suite:** 3722 passed / 31 skipped / 30 deselected (#105 baseline 3713 + 9). `ruff` clean; `scripts/fork/black_changed_lines.py origin/main` clean.
- **Real server, real models** (`scripts/fork/e2e_lazy_restore.py`: two `cli serve --continuous-batching` sessions, multi-turn conversation twice + two concurrent bursts, T=0): `Qwen3-0.6B-8bit` locally and **hybrid `Qwen3.5-4B-4bit` on the Studio** (prod venv, patched source via `PYTHONPATH`, spare port, no live route touched) — lazy output **byte-identical to the eager server's** on both passes and both bursts, 11–13 lazy restores, 0 lazy misses, no recovery, no traceback / stream-thread error.
- **Observation, not caused by this patch:** on the hybrid model turn 3 of the all-restores pass differs from the first pass ~20 tokens in — *identically on the unpatched eager server*. The first pass restored at 1090/1163 and prefilled 73 tokens, the second at 1162/1163 and prefilled 1: same maths, different chunk boundaries through the GDN recurrence, ~1e-6 logit drift, one argmax tie flipped on a 4-bit 4B. Same class the speed-lever ledger records for kernel changes. The harness reports it as a NOTE; the property this patch must keep is lazy ≡ eager. Worth one dedicated run of the fork's standard warm-vs-cold recipe on the 27B before anyone leans on "byte-identical" for multi-turn chains.
- **Live gate after arming** (config, one reload): under co-batched deep chains `lazy_restores` tracks hits, `projected_defers` > 0 appears instead of `generation_recoveries`, peaks stay under ~58 GB. Arm together with `KV_BUDGET_MB` — see the infra README.

## 107. `patch: lazy-restore-pins-ssd-fallback` — a queued request keeps its entry, or gets it back from SSD

**Files:** `vllm_mlx/batched_system_kv.py` (`_peeked` pins, `_pop_victim_locked`, pin-aware budget/make-room eviction, SSD fallback in `materialize_pending_restore`, stats), `tests/test_fork_invariants.py` (+6), `scripts/fork/e2e_lazy_restore.py` (SSD phase).

**Found by the 2026-09-21 stress test of #106** on the live Qwen3.8-27B-4bit (route idle; Tim's go-ahead): five distinct 60K-token chains seeded to fill the bag (20 GB), then the 09-17 OOM signature — one deep row decoding, a second co-batched, three more queued. **Safety passed:** 0 OOM, 0 recoveries, peak **53.3 GB** (09-17: 63.0–63.6), gate 4 made room 4× (bag 20 → 10 GB at the moment the second deep row arrived). **But 3 of 5 follow-ups re-prefilled 60K tokens cold — 12–19-minute turns.** Two causes are fixed here; the third is config:

1. **A lazy miss never tried the SSD tier (#106's gap).** A follow-up matched its chain at enqueue, waited ~10 min behind the KV budget, lost the RAM entry, and `materialize_pending_restore` went straight to `miss` — although the entry sat on SSD, 0.5–0.9 s away (measured promote of a 60K entry). The eager miss path has always probed SSD. **Fix:** at admission, if a fresh `peek` now reaches less than the position matched at enqueue, `check_ssd` → `promote_ssd` first, then `fetch` once (`lazy_ssd_fallbacks`). No disk probe while the entry is still resident (pinned by a test).
2. **Make-room and budget eviction took the entries queued requests were waiting for.** #106's LRU-touch at `peek` is not protection: an SSD promote for one chain, or gate 4 shedding two entries, pushed out the LRU — which a queued request had matched minutes earlier. **Fix:** `peek(request_id=…)` records a pin (`_peeked`), released at admission and in `discard_pending` (every abort path). `_pop_victim_locked` picks the LRU entry **nobody is waiting for**; budget enforcement falls back to a pinned one only when nothing else is left (a budget still has to hold) and never offers the entry just inserted (a promote must not evict itself). Gate 4 counts only unpinned bytes as sheddable and spares pins while shedding — a second seat *now* is not worth an 18-minute cold prefill *later*. Two deliberate exceptions: **watermark relief ignores pins** (against a real OOM everything is fair game), and the **solo guard takes pinned entries as a last resort** rather than reject (a false 503 is worse than someone else's cold prefill). `pinned_entries` in stats; must read 0 when nothing waits.
3. **Config, not code — #103's spill-queue cap is smaller than two deep entries.** The batched default `VLLM_MLX_SSD_SYSTEM_KV_MAX_QUEUED_GB=4` admits one 60K-token entry (4.1 GB); when two deep chains finish together the second spill is **dropped** (3 of 6 deep stores in the test: `spill backlog at 3.8 GB (cap 4.0 GB), dropping 60128-token spill`), so that chain never reaches SSD and is gone once evicted — which also defeats fix 1. Set **9** on deep-chain routes (two deep entries). Still safe: relief drops the backlog first (#103) and gate 4 counts it as sheddable (#106).

**Upstream:** fork-owned.

**Verification:**
- **Red then green.** 6 tests, all red with `vllm_mlx/` at `5a3b1aa`: a lazy miss is served from SSD with the position matched at enqueue (`lazy_ssd_fallbacks 1`, 0 lazy misses); no disk probe on a RAM hit; pin set at enqueue, released at admission and on abort; budget eviction prefers the unpinned entry, still holds when everything older is pinned, and never evicts the entry just inserted; make-room spares the pinned LRU, waits rather than take the last pinned entry, while relief still takes it; the solo guard takes a pinned entry instead of rejecting.
...
- **Real server, real hybrid model** (`scripts/fork/e2e_ssd_prefer.py`, `Qwen3.5-4B-4bit`, one-slot route, Studio prod venv): 6/6. Chain B's follow-up is served from the SSD snapshot, **3152/3186 tokens cached** (`ssd preferred: ram_pos=2315 ssd_len=3152 -> ssd`), against 2315 with `SSD_PREFER_GAIN=0`; chain A's cold prefill cut at the divergence (`divergence_cuts 2`) and, with A as RAM donor, chain C restores at exactly the shared 2315 tokens instead of the 2304 interval floor. (Chain C's donor in the arbitration run is B's chain, stored cold before any divergence was seen, so it restores at 2048 and learns a cut from that restore.)
- **Live A/B on the HA route** (deployed 2026-09-25 ~17:00 local, `f0672a3`; `native_eval.py --model Qwen3.6-35B-A3B-4bit --context current --prompt native --temp 0.2 --repeat 2`, 160 rows, fresh process each, route idle otherwise): old `df0af7e` **143/160, p50 1401, p95 4467 ms, 20 turns > 3.5 s, checkpoint-only restores 23/322 (7.1%)**; new (two runs) **141 and 142/160, p50 1351/1405, p95 3652/3595 ms, 9 and 10 turns > 3.5 s, checkpoint-only 2/310 and 4/624 (~0.6%, all in the first seconds after respawn)**. Accuracy unchanged within noise (the single long-lived-process baseline of 147 was an outlier vs 143 on a fresh process); zero tracebacks. **`ssd_preferred` stayed 0 live**: the benchmark's two agents keep both base prompts resident in 4 slots, so the gain here comes from the donor ranking and the divergence cut; the SSD pick is proven by the e2e above, and it is the path for real traffic that overflows the slots.
- **Watch:** production `restore at 2048|4096/…` share over a day of real HA use (baseline 4.5% on 2026-09-25) and `ssd_preferred` / `divergence_cuts` in `/v1/status`.
